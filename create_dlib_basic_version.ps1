# Create dlib Basic Face Detection Version (No shape predictor)
# This version uses only dlib's built-in frontal face detector

$workflowContent = @'
name: Test Blood Pressure Estimation (dlib Basic - No shape predictor)
...
jobs:
  test-blood-pressure-estimation-dlib-basic:
    runs-on: windows-latest
    name: Test Blood Pressure Estimation (dlib Basic Face Detection)
    ...
    steps:
      ...
      - name: Verify required files exist (dlib Basic)
        run: |
          Write-Host "Verifying required files for dlib basic blood pressure estimation test..." -ForegroundColor Yellow
          ...
          Write-Host "✓ dlib basic configuration verified (no shape predictor)" -ForegroundColor Green
          Write-Host "  - Using dlib frontal face detector only" -ForegroundColor Cyan
          Write-Host "  - Using OpenCV DNN for additional face detection" -ForegroundColor Cyan
      ...
      - name: Create dlib basic test application
        run: |
          Write-Host "Creating dlib basic blood pressure estimation test application..." -ForegroundColor Yellow
          
          # Create test directory
          New-Item -ItemType Directory -Path "test_bp_estimation_dlib_basic" -Force | Out-Null
          cd test_bp_estimation_dlib_basic
          
          # Create C# test application (dlib basic version)
          $testCode = @'
using System;
...
namespace BloodPressureTestDlibBasic
{
    ...
    class Program
    {
        ...
        {
            Console.WriteLine("=== dlib Basic Blood Pressure Estimation Test ===");
            Console.WriteLine("Note: Using dlib frontal face detector (no shape predictor)");
            Console.WriteLine("Note: Using OpenCV DNN for additional face detection");
            ...
            try
            {
                ...
                Console.WriteLine("Setting up dlib basic test environment...");
                CopyRequiredFiles();
                
                // Create estimator
                Console.WriteLine("Initializing dlib basic blood pressure estimator...");
                ...
                if (result == 0)
                {
                    ...
                    // Output results
                    Console.WriteLine();
                    Console.WriteLine("=== DLIB BASIC RESULTS ===");
                    ...
                    Console.WriteLine();
                    Console.WriteLine("=== PERFORMANCE METRICS (dlib Basic) ===");
                    ...
                    Console.WriteLine("=== DLIB BASIC PERFORMANCE ANALYSIS ===");
                    ...
                    // dlib basic benefits
                    Console.WriteLine();
                    Console.WriteLine("=== DLIB BASIC BENEFITS ===");
                    ...
                    Console.WriteLine("• Still uses dlib frontal face detector (built-in)");
                    Console.WriteLine("• Combines with OpenCV DNN for robust detection");
                    
                    // Bottleneck analysis
                    Console.WriteLine();
                    Console.WriteLine("=== BOTTLENECK ANALYSIS (dlib Basic) ===");
                    ...
                    // Recommendations
                    Console.WriteLine();
                    Console.WriteLine("=== RECOMMENDATIONS (dlib Basic) ===");
                    ...
                    Console.WriteLine();
                    Console.WriteLine("=== DLIB BASIC TEST COMPLETED SUCCESSFULLY ===");
                }
                else
                {
        ...
        {
            // dlib basic file list (excluding shape predictor)
            var files = new[]
            {
...
'@
          ...
          [System.IO.File]::WriteAllText("BloodPressureTestDlibBasic.csproj", $projectFile)
          
          Write-Host "dlib basic test application created successfully" -ForegroundColor Green
        shell: powershell

      - name: Build dlib basic test application
        run: |
          cd test_bp_estimation_dlib_basic
          dotnet restore
          ...
          if ($LASTEXITCODE -ne 0) {
            Write-Host "ERROR: Failed to build dlib basic test application" -ForegroundColor Red
            exit 1
          }
          
          Write-Host "dlib basic test application built successfully" -ForegroundColor Green
        shell: powershell

      - name: Run dlib basic blood pressure estimation test
        run: |
          cd test_bp_estimation_dlib_basic
          ...
          Write-Host "Running dlib basic blood pressure estimation test..." -ForegroundColor Yellow
          ...
          # Save output to file
          $output | Out-File -FilePath "results/dlib_basic_test_output.txt" -Encoding UTF8
          
          # Display output
          Write-Host "=== DLIB BASIC TEST OUTPUT ===" -ForegroundColor Green
          $output
          
          if ($exitCode -ne 0) {
            Write-Host "ERROR: dlib basic test failed with exit code $exitCode" -ForegroundColor Red
            exit 1
          }
          
          Write-Host "dlib basic test completed successfully" -ForegroundColor Green
        shell: powershell

      - name: Analyze dlib basic test results
        run: |
          cd test_bp_estimation_dlib_basic
          
          if (Test-Path "results/dlib_basic_test_output.txt") {
            Write-Host "=== DLIB BASIC TEST RESULTS ANALYSIS ===" -ForegroundColor Green
            
            $output = Get-Content "results/dlib_basic_test_output.txt" -Raw
            ...
            Write-Host "dlib Basic Blood Pressure Results:" -ForegroundColor Yellow
            ...
            Write-Host ""
            Write-Host "dlib Basic Performance Metrics:" -ForegroundColor Yellow
            ...
            # dlib basic benefits assessment
            Write-Host ""
            Write-Host "dlib Basic Benefits Assessment:" -ForegroundColor Yellow
            
            if ($memoryUsed -and [double]$memoryUsed -le 800) {
              Write-Host "  ✓ Lower memory usage (≤800MB) - dlib basic benefit" -ForegroundColor Green
            } elseif ($memoryUsed) {
              Write-Host "  ⚠ Memory usage could be lower" -ForegroundColor Yellow
            }
            
            if ($totalTime -and [double]$totalTime -le 25) {
              Write-Host "  ✓ Faster processing (≤25s) - dlib basic benefit" -ForegroundColor Green
            } elseif ($totalTime) {
            ...
            Write-Host ""
            Write-Host "Blood Pressure Validation (dlib Basic):" -ForegroundColor Yellow
            ...
            if ($confidence -and [double]$confidence -ge 0.6) {
              Write-Host "  ✓ Confidence level is acceptable (≥0.6) for dlib basic version" -ForegroundColor Green
            } elseif ($confidence) {
              Write-Host "  ⚠ Low confidence level (<0.6) - consider using full version" -ForegroundColor Yellow
            }
            
          } else {
            Write-Host "ERROR: dlib basic test output file not found" -ForegroundColor Red
          }
        shell: powershell

      - name: Upload dlib basic test results
        uses: actions/upload-artifact@v4
        with:
          name: blood-pressure-test-results-dlib-basic-${{ github.sha }}
          path: test_bp_estimation_dlib_basic/results/
          retention-days: 30
          if-no-files-found: error

      - name: Upload dlib basic test application
        uses: actions/upload-artifact@v4
        with:
          name: blood-pressure-test-app-dlib-basic-${{ github.sha }}
          path: test_bp_estimation_dlib_basic/bin/Release/net6.0/
          retention-days: 30
          if-no-files-found: error

      - name: Generate dlib basic test report
        run: |
          Write-Host "=== DLIB BASIC BLOOD PRESSURE ESTIMATION TEST REPORT ===" -ForegroundColor Green
          ...
          Write-Host "Test Duration: ${{ env.TEST_DURATION }} seconds" -ForegroundColor Cyan
          Write-Host ""
          Write-Host "dlib Basic Configuration:" -ForegroundColor Yellow
          Write-Host "- dlib frontal face detector (built-in)" -ForegroundColor Cyan
          Write-Host "- OpenCV DNN face detection (additional)" -ForegroundColor Cyan
          ...
          Write-Host "- dlib basic test application: test_bp_estimation_dlib_basic/bin/Release/net6.0/" -ForegroundColor Cyan
          Write-Host "- dlib basic test results: test_bp_estimation_dlib_basic/results/" -ForegroundColor Cyan
          Write-Host "- Performance logs: Available in artifacts" -ForegroundColor Cyan
          Write-Host ""
          Write-Host "Next Steps:" -ForegroundColor Yellow
          Write-Host "1. Download dlib basic test results artifact for detailed analysis" -ForegroundColor Cyan
          ...
          Write-Host "4. Consider using dlib basic version for deployment" -ForegroundColor Cyan
        shell: powershell
'@
...
# Write the dlib basic workflow file
[System.IO.File]::WriteAllText(".github/workflows/test-blood-pressure-estimation-dlib-basic.yml", $workflowContent)

Write-Host "dlib Basic Blood Pressure Estimation Test Workflow created successfully!" -ForegroundColor Green
Write-Host "File location: .github/workflows/test-blood-pressure-estimation-dlib-basic.yml" -ForegroundColor Cyan
Write-Host ""
Write-Host "Key differences from full version:" -ForegroundColor Yellow
Write-Host "- Uses dlib frontal face detector (built-in)" -ForegroundColor Cyan
Write-Host "- Uses OpenCV DNN for additional face detection" -ForegroundColor Cyan
Write-Host "- Excludes shape predictor (~100MB saved)" -ForegroundColor Cyan
...
Write-Host "3. Select 'Test Blood Pressure Estimation (dlib Basic - No shape predictor)'" -ForegroundColor Cyan
